...
void insertSymbolEntry(char *token, int lineNumber, int startColumn, int tokenScope, char *type, char *value, char* length)
{
        ...
        if (DEBUG_MODE)
            //printf("\033[0;32mAdded a new Entry to Symbol Table.\033[0;0m\n");
            //printf("%d",functions[functionid].symbolCount );
...
void updateSymbolEntry(char *token, int lineNumber, int startColumn, int tokenScope, char *type, char *value)
{
    ...
    if (DEBUG_MODE)
        //printf("\033[0;32mIdenitifier exists. Symbol Table updated.%s\033[0;0m\n", value);
    return;
...
void printSymbolTable()
{
    ...
    {
        printf("\nFunction Name \t\033[0;35mReturn Type\033[0;33m\t\tParameters\tSymbolCount\033[0;0m\n");
        printf("\n%s\t\033[0;35m%s\033[0;33m\t\t\t\t%s\t\t%d\033[0;00m\t\n",functions[j].name,functions[j].returntype,functions[j].params,functions[j].symbolCount);
        ...
        printf("\t\t\t\t\t\t\t \033[0;31mSYMBOL TABLE for %s function\033[0;0m \n",functions[j].name);
